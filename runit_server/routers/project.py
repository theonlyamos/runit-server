...
async def update_user_project(request: Request):
    # user_id = request.session['user_id']
    return templates.TemplateResponse('projects/index.html', page='projects', projects=[])


@project.get('/editor/{project_id}/')
async def user_project_editor(request: Request, project_id):
    # Define the route for the editor page
    project = Project.get(project_id)
    project = project.json()
    if project:
        return templates.TemplateResponse('projects/editor.html',context={
            'request': request, 'page': 'projects',
            'project': project, 'fullscreen': True})
    else:
        flash(request, PROJECT_404_ERROR, 'danger')
        return RedirectResponse(request.url_for(PROJECT_INDEX_URL_NAME))
...
async def user_project_files(request: Request, project_id):
    old_curdir = os.curdir
    ...
    project = project.json()
    if not project:
        return JSONResponse({'status': 'error', 'message': PROJECT_404_ERROR})
    
    return JSONResponse({'status': 'success', 'files': files, 'project': project})

@project.get('/file/{project_id}/')
async def get_project_file(request: Request, project_id: str, file: str):
    old_curdir = os.curdir
...
async def update_project_file(request: Request, project_id: str, file: str):
    old_curdir = os.curdir
